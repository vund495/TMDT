    ...
    <label className={cn("block", className)}>
      ...
      {trailing ? (
        <div className="relative">
          <input className={cn(inputClass, error && "border-red-400 focus:border-red-400 focus:ring-red-200", input, "pr-10")} {...rest} />
          <span className="absolute inset-y-0 right-0 flex items-center pr-3">{trailing}</span>
        </div>
      ) : (
        <input className={cn(inputClass, error && "border-red-400 focus:border-red-400 focus:ring-red-200", input)} {...rest} />
      )}
